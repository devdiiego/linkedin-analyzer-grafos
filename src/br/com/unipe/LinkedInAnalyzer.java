...
public class LinkedInAnalyzer {
    ...
    public List<Map.Entry<String, Integer>> sugerirConexoes(String nomePessoa) {
        ...
        // Navega para os amigos dos amigos (2º grau)
        for (Vertice amigo : contatosDiretos) {
            for (Aresta a : amigo.getArestas()) {
                Vertice amigoDoAmigo = a.getDestino();

                // Regras: Não pode ser o próprio usuário, nem contato direto
        ...
        // Ordena em ordem decrescente pela quantidade de amigos em comum
        resultado.sort((e1, e2) -> e2.getValue().compareTo(e1.getValue()));

        return resultado;
    }

    // Missão 3: Grau de Separação (Usando BFS - Busca em Largura para menor número de saltos)
    ...
    // Missão 4: Rota e Custo de Maior Afinidade (Delegado ao algoritmo de Dijkstra do Grafo)
    ...
    // Missão 5: Mapear Grupos Isolados (Sub-redes usando DFS - Busca em Profundidade)
